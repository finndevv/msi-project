...
android {
    ...
    defaultConfig {
        ...
        ndk {
            // Native components (proot/talloc/android-shmem) are built from
            // the vendored sources in src/main/native — no prebuilt .so files.
    ...
    externalNativeBuild {
        cmake {
            // Builds libproot.so, libproot-loader.so, libtalloc.so and
            // libandroid-shmem.so from source (see src/main/native/README.md).

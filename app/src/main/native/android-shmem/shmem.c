...
#include <unistd.h>
#include <paths.h>
#include <fcntl.h> /* open, O_RDWR — MSI build: not pulled in by bionic headers
                      (needed with clang >= 16, where implicit function
                      declarations are hard errors in C99+). The only change
                      to the upstream v0.7 source. */

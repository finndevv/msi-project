...
object PRootEngine {
    ...
    private fun loaderPath(ctx: Context): String? {
        // Preferred: the loader built from source and packaged in jniLibs
        // (libproot-loader.so), extracted to nativeLibraryDir — the only
        // location from which exec() is permitted on Android 10+ (targetSdk
        // 29+ SELinux blocks executing files from app-writable storage).
        val nativeLoader = File(ctx.applicationInfo.nativeLibraryDir, "libproot-loader.so")
        if (nativeLoader.isFile) return nativeLoader.absolutePath
        // Legacy fallback: loader extracted from assets/proot/ by the app.

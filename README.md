...
```
Requires JDK 17, an Android SDK with `platforms;android-35` + `build-tools;35.0.0`, and an
Android NDK (the native components in `app/src/main/native/` are compiled by CMake during
the build; CMake 3.22.1 is fetched automatically by the Gradle plugin).
...
- All native components are **built from source** during the Gradle build
  (`app/src/main/native/`, wired via `externalNativeBuild` CMake — see
  `app/src/main/native/README.md` for provenance, versions and licenses):
  - PRoot **5.1.107.92** (Termux fork) → `libproot.so` + `libproot-loader.so`,
  - talloc **2.4.3** → `libtalloc.so` (soname `libtalloc.so.2`),
  - libandroid-shmem **0.7** → `libandroid-shmem.so`,
  for `arm64-v8a`, `armeabi-v7a`, `x86_64` — extracted to `nativeLibraryDir`
  automatically.
